...
import familyshopper.shared.generated.resources.warning_text_changed
import familyshopper.shared.generated.resources.warning_text_deleted
...
interface SynchronizeListsUseCase {
        ...
        try {
            if (store.getGroupUUID().isNotBlank()) {
                ...
                val deletedListKeys: Set<String> =
                    localListInfo.filter { list -> !remoteListsInfo.containsKey(list.key) && list.value.ownerUuid != userUuid }.keys
                ...
                if (countUpdated != 0) completeText.append(
                    getString(
                        Res.string.warning_text_changed,
                        countUpdated.toString()
                    )
                )
                if (countDeleted != 0) completeText.append(
                    getString(
                        Res.string.warning_text_deleted,
